...
export async function authorizeEventGroups(
	...
	for (const groupId of groupIds) {
		const group =
			groupId === groupIds[0] ? firstGroup : await getGroupById(groupId);
		if (!group) throw new ORPCError("NOT_FOUND");
		const site =
			groupId === groupIds[0]
				? firstSite
				: await getSiteById(group.siteId);
		if (!site) throw new ORPCError("NOT_FOUND");
		const area =
			groupId === groupIds[0]
				? firstArea
				: await getAreaById(site.areaId);
		...
		if (!isOwner) {
			if (userSiteIds.length > 0) {
				if (!(await canAccessSite(userId, group.siteId)))
					throw new ORPCError("FORBIDDEN");
