...
export const submitMentorApplication = publicProcedure
  ...
  .handler(async ({ input }) => {
    ...
    const emailCount = await countRecentMentorApplicationsByEmail(input.email, ONE_HOUR_MS);
    if (emailCount >= EMAIL_LIMIT) throw new ORPCError("TOO_MANY_REQUESTS");

    // Validate profile field IDs belong to this org
    const validProfileFields = await getCollectInApplicationFieldsByOrg(org.id);
    const validProfileFieldIds = new Set(validProfileFields.map((f: { id: string }) => f.id));
    ...
    const safeProfileFieldValues = input.profileFieldValues?.filter((v) => validProfileFieldIds.has(v.customFieldId));

    const { orgSlug, formFieldValues: _fv, profileFieldValues: _pfv, ...rest } = input;
    ...
    try {
      result = await createMentorApplication({
        ...
        ...rest,
        formFieldValues: safeFormFieldValues?.length ? safeFormFieldValues : undefined,
