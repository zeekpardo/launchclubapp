...
const PERSON_PROFILE_STRING_KEYS: ReadonlySet<keyof PersonProfileUpdates> =
	new Set([
		"firstName",
		"lastName",
		"email",
...
function buildPersonProfileUpdates(
	...
	for (const ff of formFields) {
		...
		if (ff.profileFieldKey === "dateOfBirth") {
			...
			PERSON_PROFILE_STRING_KEYS.has(
				ff.profileFieldKey as keyof PersonProfileUpdates,
			)
		) {
...
export const publicSubmitForm = publicProcedure
	...
	.handler(async ({ input, context }) => {
		...
		enforceRateLimit(
			`forms-submit:${getClientIp(context.headers)}`,
			20,
			10 * 60 * 1000,
		);
		...
		if (form.status === "UNPUBLISHED") {
			throw new ORPCError("NOT_FOUND", {
				message: "This form isn't available.",
			});
		}
		...
		const targetSiteId =
			input.siteId && formSiteIds.includes(input.siteId)
				? input.siteId
				: formSiteIds[0];

		if (!targetSiteId) {
			throw new ORPCError("BAD_REQUEST", {
				message: "No site is linked to this form.",
			});
		}
		...
		if (form.type === "MENTOR") {
			...
			const firstName =
				profileValue(submittedFields, allFields, "firstName") ?? "";
			const lastName =
				profileValue(submittedFields, allFields, "lastName") ?? "";
			const email =
				profileValue(submittedFields, allFields, "email") ?? "";
			const phone = profileValue(submittedFields, allFields, "phone");
			const addressLine1 = profileValue(
				submittedFields,
				allFields,
				"addressLine1",
			);
			const city = profileValue(submittedFields, allFields, "city");
			const stateProvince = profileValue(
				submittedFields,
				allFields,
				"stateProvince",
			);
			const postalCode = profileValue(
				submittedFields,
				allFields,
				"postalCode",
			);
			...
			if (!email) {
				throw new ORPCError("BAD_REQUEST", {
					message: "Email is required for mentor applications.",
				});
			}
			...
			const mentorAppFieldValues = buildAppFieldValues(
				submittedFields,
				allFields,
			);
			const mentorCustomFieldValues = buildCustomFieldValues(
				submittedFields,
				allFields,
			);
			...
			const result = await createMentorApplication({
				...
				postalCode,
				country,
				formFieldValues: mentorAppFieldValues.length
					? mentorAppFieldValues
					: undefined,
				profileFieldValues: mentorCustomFieldValues.length
					? mentorCustomFieldValues
					: undefined,
			});
		...
		const parentFormFields = allFields.filter(
			(f) =>
				f.targetPersonType === "PARENT" || f.targetPersonType === null,
		);
		...
		const parentAddressLine1 = profileValue(
			parentFields,
			parentFormFields,
			"addressLine1",
		);
		const parentCity = profileValue(parentFields, parentFormFields, "city");
		const parentStateProvince = profileValue(
			parentFields,
			parentFormFields,
			"stateProvince",
		);
		const parentPostalCode = profileValue(
			parentFields,
			parentFormFields,
			"postalCode",
		);
		const parentCountry = profileValue(
			parentFields,
			parentFormFields,
			"country",
		);
		...
		if (parentEmail) {
			const emailCount = await countRecentApplicationsByEmail(
				parentEmail,
				ONE_HOUR_MS,
			);
			if (emailCount >= EMAIL_LIMIT) {
		...
		const siteCount = await countRecentApplicationsBySite(
			targetSiteId,
			ONE_HOUR_MS,
		);
		...
		const childrenData = students.map((studentEntry) => {
			...
			const lastName =
				...
				"";
			const birthdayStr = profileValue(
				studentEntry,
				studentFormFields,
				"dateOfBirth",
			);
			const grade = profileValue(
				studentEntry,
				studentFormFields,
				"grade",
			);

			const childFormFieldValues = buildAppFieldValues(
				studentEntry,
				studentFormFields,
			);
			const childCustomFieldValues = buildCustomFieldValues(
				studentEntry,
				studentFormFields,
			);
			...
			return {
				...
				formFieldValues: childFormFieldValues.length
					...
					: undefined,
				profileFieldValues: childCustomFieldValues.length
		...
		const application = await createApplication({
			...
			profileFieldValues: appProfileFieldValues.length
				? appProfileFieldValues
				: undefined,
		});
		...
		if (parentEmail) {
			const parentProfileUpdates = buildPersonProfileUpdates(
				parentFields,
				parentFormFields,
			);
			...
			for (const studentEntry of students) {
				...
				const studentProfileUpdates = buildPersonProfileUpdates(
					studentEntry,
					studentFormFields,
				);
				...
				const existingStudent = parentHouseholdId
					? await db.person.findFirst({
							where: {
								...
								firstName: {
									equals: studentFirstName,
									mode: "insensitive",
								},
								lastName: {
									equals: studentLastName,
									mode: "insensitive",
								},
								personType: "STUDENT",
								...(dob ? { dateOfBirth: dob } : {}),
				...
				const studentCustomFieldValues = buildCustomFieldValues(
					studentEntry,
					studentFormFields,
				);
