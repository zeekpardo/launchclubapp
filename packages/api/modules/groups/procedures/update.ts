...
import { getGroupById, updateGroup, getSiteById, getAreaById, getUserSiteIds } from "@repo/database";
  ...
  .handler(async ({ input, context }) => {
    ...
    if (!isOwner) {
      ...
      if (userSiteIds.length > 0) {
        ...
        if (!(await canManageGroup(context.user.id, input.id))) throw new ORPCError("FORBIDDEN");
      }
    }
    const { id, ...data } = input;
