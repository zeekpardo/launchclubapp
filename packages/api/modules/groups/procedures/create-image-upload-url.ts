...
import { getGroupById, getSiteById, getAreaById } from "@repo/database";
...
export const createGroupImageUploadUrl = protectedProcedure
  .route({
    ...
    description: "Create a signed upload URL to upload a group image to the storage bucket",
  })
  ...
  .handler(async ({ context: { user }, input: { groupId } }) => {
    ...
    const membership = await verifyOrganizationMembership(area.organizationId, user.id);
