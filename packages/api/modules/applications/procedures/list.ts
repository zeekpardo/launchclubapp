...
import { getApplicationsByOrganization, getApplicationsByUserSites } from "@repo/database";
...
export const listApplications = protectedProcedure
  ...
  .handler(async ({ input, context }) => {
    ...
    if (isOwner) return getApplicationsByOrganization(input.organizationId, input.status);
