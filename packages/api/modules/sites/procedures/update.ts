...
export const updateSiteProcedure = protectedProcedure
  ...
  .handler(async ({ input, context }) => {
    ...
    if (!isOwner) {
      ...
      if (!(await canAccessSite(context.user.id, input.id))) throw new ORPCError("FORBIDDEN");
    }
    const { id, applicationDeadline, ...rest } = input;
    return updateSite(id, {
      ...rest,
      applicationDeadline: applicationDeadline === null ? null : applicationDeadline ? new Date(applicationDeadline) : undefined,
    });
  });
