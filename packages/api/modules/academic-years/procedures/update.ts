...
import { deactivateOtherAcademicYears, getAcademicYearById, updateAcademicYear } from "@repo/database";
...
export const updateAcademicYearProcedure = protectedProcedure
  .route({ method: "PATCH", path: "/academic-years/{id}", tags: ["Academic Years"] })
  .input(updateAcademicYearSchema)
  .handler(async ({ input, context }) => {
    const year = await getAcademicYearById(input.id);
    if (!year) throw new ORPCError("NOT_FOUND");
    const membership = await verifyOrganizationMembership(year.organizationId, context.user.id);
