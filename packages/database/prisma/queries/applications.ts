						...
						create: children.map(
							({
								...
								...(childPfv?.length
									...
									: {}),
								...(childFfv?.length
									? { formFieldValues: { create: childFfv } }
									: {}),
							}),
						),
					}
...
export async function migrateApplicationToPeople(
	...
	const application = await db.application.findUnique({
		...
		include: {
			children: { include: { profileFieldValues: true } },
				...
				await tx.customFieldValue.upsert({
					where: {
						customFieldId_personId: {
							customFieldId: pfv.customFieldId,
							personId: childPerson.id,
						},
					},
					create: {
...
export async function getAllApplications({
	...
	return db.application.findMany({
		where: query
			...
			: undefined,
		include: {
			site: { include: { area: { include: { organization: true } } } },
			children: true,
		},
		orderBy: { createdAt: "desc" },
		take: limit,
...
export async function getApplicationsByUserSites(
	userId: string,
	status?: string,
) {
	return db.application.findMany({
...
export async function countRecentApplicationsBySite(
	siteId: string,
	windowMs: number,
) {
	const since = new Date(Date.now() - windowMs);
