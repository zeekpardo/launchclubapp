...
export function GroupPicker({ groups, selectedGroupIds, onToggle, error }: GroupPickerProps) {
  ...
  const toggleArea = (areaId: string) => {
    ...
    if (removing) {
      const sitesInArea = groups.filter((g) => g.site?.area?.id === areaId).map((g) => g.siteId);
  ...
  const toggleSite = (siteId: string) => {
    ...
    setSelectedSiteIds((prev) => (removing ? prev.filter((x) => x !== siteId) : [...prev, siteId]));
    if (removing) {
          ...
          <div className="flex flex-wrap gap-2">
            {areas.map((area) => {
              ...
              return (
                ...
                >
                  <input type="checkbox" className="sr-only" checked={checked} onChange={() => toggleArea(area.id)} />
                  {area.name}
                </label>
              );
            })}
          ...
          <div className="flex flex-wrap gap-2">
            {visibleSites.map((site) => {
              ...
              return (
                ...
                >
                  <input type="checkbox" className="sr-only" checked={checked} onChange={() => toggleSite(site.id)} />
                  {site.name}
                </label>
              );
            })}
              ...
              {siteGroups.map((group) => (
                <label
                  ...
                  className={cn(
                    ...
                    selectedGroupIds.includes(group.id) ? "border-primary bg-primary/5" : "hover:bg-muted/50",
                  )}
                >
                  <input
                    type="checkbox"
