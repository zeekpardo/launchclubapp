...
export function AreasAndSites() {
	...
	const [appSettingsDialog, setAppSettingsDialog] = useState<{
		open: boolean;
		site?: AppSettingsSite;
	}>({ open: false });
	...
	return (
		<>
			...
			<div className="space-y-3">
				{areas?.map((area) => (
					<AreaRow
						...
						onAddSite={(areaId) =>
							setSiteDialog({ open: true, defaultAreaId: areaId })
						}
						onEditSite={(s) =>
							setSiteDialog({ open: true, site: s })
						}
						onAddGroup={(siteId) =>
							setGroupDialog({
								open: true,
						...
						onAppSettings={(s) =>
							setAppSettingsDialog({
								...
								site: {
									...
									slug: s.slug,
									acceptApplications:
										s.acceptApplications ?? true,
									applicationDeadline: s.applicationDeadline
										? String(s.applicationDeadline)
										: null,
								},
							})
						}
			...
			<AreaDialog
				...
				onDelete={
					areaDialog.area
						? () => setDeleteAreaId(areaDialog.area!.id)
						: undefined
				}
			/>
			...
			<SiteDialog
				...
				onDelete={
					siteDialog.site
						? () => setDeleteSiteId(siteDialog.site!.id)
						: undefined
				}
			/>
			...
			<GroupDialog
				...
				onDelete={
					groupDialog.group
						? () => setDeleteGroupId(groupDialog.group!.id)
						: undefined
				}
			/>
			...
			<AlertDialog
				open={!!deleteAreaId}
				onOpenChange={(open) => !open && setDeleteAreaId(undefined)}
			>
				<AlertDialogContent>
					<AlertDialogHeader>
						<AlertDialogTitle>
					...
					<AlertDialogFooter>
						<AlertDialogCancel>
							{t("launchclub.areas.form.cancel")}
						</AlertDialogCancel>
			...
			<AlertDialog
				open={!!deleteSiteId}
				onOpenChange={(open) => !open && setDeleteSiteId(undefined)}
			>
				<AlertDialogContent>
					<AlertDialogHeader>
						<AlertDialogTitle>
			...
			<AlertDialog
				open={!!deleteGroupId}
				onOpenChange={(open) => !open && setDeleteGroupId(undefined)}
			>
				<AlertDialogContent>
					<AlertDialogHeader>
						<AlertDialogTitle>
