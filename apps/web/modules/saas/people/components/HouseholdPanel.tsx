...
export function HouseholdPanel({ personId, householdId }: HouseholdPanelProps) {
	...
	const handleHouseholdCreated = async (newHouseholdId: string) => {
		try {
			...
			queryClient.invalidateQueries(
				orpc.households.get.queryOptions({
					input: { id: newHouseholdId },
				}),
			);
		} catch {
			toastError(
				t("launchclub.households.notifications.autoAssignError"),
			);
		}
	};
		...
		<>
						...
						{isLoading ? (
							...
							<div className="space-y-4">
								...
								>
									...
									{t(
										"launchclub.households.removeFromHousehold",
									)}
								</Button>
							</div>
						)}
				...
				<DialogContent>
					...
					<DialogFooter>
						...
						<Button
							...
							disabled={
								!selectedHouseholdId || updatePerson.isPending
							}
						>
			...
			>
				<DialogContent>
					<DialogHeader>
						<DialogTitle>
							{t("launchclub.households.addPersonDialog.title")}
						</DialogTitle>
					</DialogHeader>
					<Select
					...
					>
						<SelectTrigger>
							<SelectValue
								placeholder={t(
									"launchclub.households.addPersonDialog.selectPlaceholder",
								)}
							/>
						</SelectTrigger>
						<SelectContent>
					...
					<DialogFooter>
						...
						<Button
							...
							disabled={
								!selectedPersonId || updatePerson.isPending
							}
						>
