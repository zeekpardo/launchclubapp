...
export function GroupResourcesTab({ groupId }: GroupResourcesTabProps) {
	...
	const dialogEvents =
		events?.map((e) => ({ id: e.id, name: e.name, startsAt: e.startsAt })) ??
		[];

	return (
		<div className="space-y-4">
			<div className="flex items-center justify-between">
				<h2 className="text-xl font-semibold">{t("launchclub.resources.title")}</h2>
				...
				<div className="divide-y">
					{resources?.map((resource) => (
						<div key={resource.id} className="flex items-start gap-3 py-4">
							...
							<div className="min-w-0 flex-1">
								...
								{resource.event && (
									<Badge className="mt-2 normal-case" status="info">
					...
					{resources?.length === 0 && (
						<div className="flex flex-col items-center gap-3 py-12 text-muted-foreground">
							...
							<p className="text-sm">{t("launchclub.resources.empty")}</p>
						</div>
					)}
				</div>
			)}
