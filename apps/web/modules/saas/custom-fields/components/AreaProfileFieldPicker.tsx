...
export function AreaProfileFieldPicker({
	...
	const [selectedField, setSelectedField] = useState<ProfileField | null>(
		null,
	);

	const linked = (allFields as ProfileField[]).filter(
		(f) => f.collectInApplication,
	);
	const available = (allFields as ProfileField[]).filter(
		(f) => !f.collectInApplication,
	);
		...
		<div className="space-y-4">
				...
				<DropdownMenu>
					<DropdownMenuTrigger asChild>
						<Button
							...
							variant="outline"
							className="shrink-0"
							disabled={available.length === 0 && !isLoading}
						>
					...
					<DropdownMenuContent align="end" className="w-56">
						{available.length === 0 ? (
							<div className="px-3 py-2 text-xs text-muted-foreground">
								All fields are already added, or no fields
								exist.{" "}
								<Link
			...
			{isLoading ? (
				...
				<p className="text-sm text-muted-foreground">
					No profile fields added yet. Click "Add Profile Field" to
					pick from your{" "}
					<Link
						href={`/app/${organizationSlug}/settings/custom-fields`}
				...
				<div
					className={`grid gap-4 ${selectedField ? "grid-cols-1 md:grid-cols-2" : ""}`}
				>
					<div className="space-y-2">
						{linked.map((field) => (
							<div
