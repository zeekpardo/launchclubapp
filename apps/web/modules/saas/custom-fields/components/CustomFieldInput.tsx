...
function FileFieldInput({
  ...
  const getUploadUrl = useMutation(orpc.customFields.getUploadUrl.mutationOptions());
  ...
  const handleFileChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    setUploading(true);
    try {
      const { signedUploadUrl, path } = await getUploadUrl.mutateAsync({ customFieldId, personId });
  ...
  return (
    <div className="flex items-center gap-2">
      ...
      >
        ...
        <span className="ml-1.5">{uploading ? "Uploading…" : value ? "Replace" : "Upload file"}</span>
      </Button>
      {value && (
        <a
