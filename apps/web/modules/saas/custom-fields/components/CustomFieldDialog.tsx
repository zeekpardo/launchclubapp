  ...
  async function onSubmit(values: FormValues) {
    ...
    try {
      if (isEditing && field) {
        ...
        toastSuccess(t("launchclub.customFields.notifications.updated"));
      } else {
        ...
        toastSuccess(t("launchclub.customFields.notifications.created"));
      }
      onOpenChange(false);
    } catch {
          ...
          <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-4">
                    ...
                    <SelectContent>
                      {FIELD_TYPES.map((type) => (
                        <SelectItem key={type} value={type}>
                          {t(`launchclub.customFields.form.types.${type}`)}
                        </SelectItem>
                      ))}
                    </SelectContent>
                  </Select>
            ...
            {watchType === "SELECT" && (
              <FormField
                ...
                render={({ field: f }) => (
                  <FormItem>
                    <FormLabel>{t("launchclub.customFields.form.options")}</FormLabel>
                    <FormControl>
                      <Textarea
                        {...f}
                        rows={4}
              ...
              render={({ field: f }) => (
                <FormItem className="flex items-center gap-2 space-y-0">
                  ...
                  <FormLabel className="font-normal cursor-pointer">
                    {t("launchclub.customFields.form.required")}
                  </FormLabel>
                </FormItem>
              )}
            />
