...
export function ProfileFieldEditorPanel({
  ...
  const handleSwap = async () => {
    ...
    try {
      ...
      await updateField.mutateAsync({ id: field.id, collectInApplication: false });
      await updateField.mutateAsync({ id: swapTo, collectInApplication: true });
  ...
  return (
    <div className="rounded-lg border bg-card p-4 space-y-4">
      ...
      <div className="space-y-1">
        <p className="text-xs text-muted-foreground uppercase tracking-wide">Current field</p>
        <p className="text-sm font-medium">{field.name}</p>
        <p className="text-xs text-muted-foreground capitalize">{field.type.toLowerCase()}</p>
      </div>
